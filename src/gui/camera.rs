...
pub fn render_camera(world: &World, res: &Resources, ctx : &mut Rltk) {
    ...
    for ty in min_y ..= max_y {
        ...
        for tx in min_x ..= max_x {
            if tx >= 0 && tx < map_width && ty >= 0 && ty < map_height {
                ...
                if gamemode == GameMode::Sim || map.revealed_tiles[idx] {
                    let (glyph, mut fg, mut bg) = get_tile_glyph(idx, &*map);

                    if gamemode != GameMode::Sim && !map.visible_tiles[idx] {
                        fg.scale(0.5);
                        bg.scale(0.5);
                    }
