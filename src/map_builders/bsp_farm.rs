...
use super::{MapBuilder, Map, Rect, TileType, Position, common::apply_room_to_map};
...
impl BspFarmBuilder {
    pub fn new(new_depth : i32) -> BspFarmBuilder {
        BspFarmBuilder{
    ...
    fn build(&mut self) {
        ...
        self.starting_position = Position{ ps:vec![Point{x: start.0, y: start.1}] };
    ...
    fn add_subrects_recursive(&mut self, rect : Rect, rng : &mut RandomNumberGenerator) {
        ...
        if split <= 2 {
            ...
            self.rects.push( h1 );
            if half_width > MIN_ROOM_SIZE { self.add_subrects_recursive(h1, rng); }
            let h2 = Rect::new( rect.x1 + half_width, rect.y1, half_width, height );
            self.rects.push( h2 );
            if half_width > MIN_ROOM_SIZE { self.add_subrects_recursive(h2, rng); }
        } else {
            // Vertical split
            let v1 = Rect::new( rect.x1, rect.y1, width, half_height-1 );
            self.rects.push(v1);
            if half_height > MIN_ROOM_SIZE { self.add_subrects_recursive(v1, rng); }
            let v2 = Rect::new( rect.x1, rect.y1 + half_height, width, half_height );
            self.rects.push(v2);
            if half_height > MIN_ROOM_SIZE { self.add_subrects_recursive(v2, rng); }
        }
    }
}