...
//     let mut store = gs.world.borrow::<AllStoragesViewMut>().unwrap();

//     let mut target_item: Option<EntityId> = None;

//     gs.world.run(|vpos: View<Position>, vitem: View<Item>| {
//         for (id, (pos, _)) in (&vpos, &vitem).iter().with_id() {
...
//         Some(item) => {
//             store.add_component(player_id, WantsToPickupItem {
...
pub fn reveal_map(gs: &mut State){
    ...
    if let Ok(space) = &mut world.get::<SpatialKnowledge>(player_id) {
